...
pub async fn execute_command(cmd: &Value, state: &mut DaemonState) -> Value {
    ...
    // popup, a closed tab, or a crash. Journaled on every change so external
    // consumers can mirror tab state without polling.
...
async fn handle_window_bounds(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
    ...
    let bounds = if let Some(window_state) = cmd.get("windowState").and_then(|v| v.as_str()) {
        ...
        if mgr.headless && (window_state == "minimized" || window_state == "maximized") {
            return Err(format!(
